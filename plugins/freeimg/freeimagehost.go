...
func (f *FreeImageService) Upload(filename string) (string, error) {
	return freeImgUpload(filename)
}

// freeImgUpload 函数用于将本地文件上传到 Freeimage.host 并返回图片的显示 URL
func freeImgUpload(uploadFile string) (string, error) {
	apiKey := global.Config.PictureBed.ApiKey
	// 创建一个字节缓冲区用于存储请求体
	...
	if err := bodyWriter.WriteField("key", apiKey); err != nil {
		global.Log.Error("写入 API 密钥字段失败:", err)
		return "", err
	}
	if err := bodyWriter.WriteField("type", "file"); err != nil {
		global.Log.Error("写入 TYPE 密钥字段失败:", err)
		return "", err
	}
	if err := bodyWriter.WriteField("action", "upload"); err != nil {
		global.Log.Error("写入 action 密钥字段失败:", err)
		return "", err
	}
	...
	if _, err := io.Copy(fileWriter, file); err != nil {
		global.Log.Error("复制文件内容失败:", err)
		return "", err
	}

	// 关闭 multipart 写入器
	if err := bodyWriter.Close(); err != nil {
		global.Log.Error("关闭 multipart 写入器失败:", err)
		return "", err
	}
	...
	if err := json.Unmarshal(response.Body(), &res); err != nil {
		global.Log.Error("解析响应 JSON 失败", err)
		return "", err
	}
	// 检查响应中是否包含 image 字段
	imageData, ok := res["image"].(map[string]any)
	if !ok {
		global.Log.Error("响应缺少image字段", err)
		return "", err
	}
	...
	if !ok {
		global.Log.Error("响应缺少display_url字段", err)
		return "", err
	}
